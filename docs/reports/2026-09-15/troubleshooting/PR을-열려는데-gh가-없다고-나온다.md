...
> 2026-09-15. 「작업을 마치면 Claude 가 PR 까지 연다」로 규율을 바꿔 보다가 부딪힌 자리다.
>
> 🔴 **그 규율 변경은 같은 날 사용자가 되돌렸다.** 여기 적힌 벽 — `gh` 로그인은 브라우저와
> 코드 입력이 필요해서 **비대화형 세션에서 끝낼 수 없다** — 때문에, PR 한 번 열자고 사람이
> 로그인해 주는 것보다 **사람이 PR 을 여는 편이 단계가 적다.** 그래서
> [규율](../../../../CLAUDE.md)은 그대로다: **Claude 는 push 까지, PR 과 머지는 사용자.**
>
> 그래도 이 문서는 남긴다. `gh` 는 이미 깔려 있고(2.100.0), 다음에 누군가
> 같은 길을 걸으면 **여기서 막힌다는 사실 자체가 답**이다.
>
